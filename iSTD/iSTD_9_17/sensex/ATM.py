...
class algoLogic(optOverNightAlgoLogic):
    ...
    def run(self, startDate, endDate, baseSym, indexSym):
        ...
        for timeData in df.index:
            ...
            if (self.humanTime.time() == time(9, 17)) or (reFetch == True):
                open_epoch = lastIndexTimeData[1]

                callSym = self.getCallSym(self.timeData, baseSym, float(df.at[lastIndexTimeData[1], "c"]), Currentexpiry)
                putSym = self.getPutSym(self.timeData, baseSym, float(df.at[lastIndexTimeData[1], "c"]), Currentexpiry)
...
if __name__ == "__main__":
    ...
    devName = "AM"
    strategyName = "iSTD_SS_A"
    version = "v1"

    startDate = datetime(2024, 1, 1, 9, 15)
    endDate = datetime(2025, 1, 22, 15, 30)
