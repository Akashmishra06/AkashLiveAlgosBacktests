...
class algoLogic(optOverNightAlgoLogic):
    ...
    def run(self, startDate, endDate, baseSym, indexSym):
        ...
        for timeData in df.index:
            ...
            if not self.openPnl.empty and lastIndexTimeData[1] in df.index:
                for index, row in self.openPnl.iterrows():
                    ...
                    try:
                        ...
                        if row['PositionStatus'] == 1:
                            if symSide == "PE" and df.at[lastIndexTimeData[1], "putSell"] == "putSell":
                                exitType = f"PutBuyExit"
                                self.exitOrder(index, exitType)
            ...
            hedge_positions = self.openPnl[self.openPnl['PositionStatus'] == 1]
            putTradeCounter_Buy = hedge_positions['Symbol'].str[-2:].value_counts().get('PE', 0)

            if lastIndexTimeData[1] in df.index and self.humanTime.time() < time(15, 15) and self.humanTime.time() > time(9, 17):

                if df.at[lastIndexTimeData[1], "putExit"] == "putExit" and putTradeCounter_Buy == 0:
                    ...
                    try:
                        ...
                        self.entryOrder(data_put_atm["c"], put_sym_atm, lotSize, "BUY", {"Expiry": expiryEpoch})
                        self.strategyLogger.info(f"{self.humanTime}, BUY Entry order: {put_sym_atm}, entryPrice: {data_put_atm['c']}")
                    except Exception as e:
                        self.strategyLogger.info(f"{self.humanTime} Exception occurred in PUTBUY  branch: {e}")
                        ...
                        if data_call_atm is not None and data_put_atm is not None:
                            ...
                            while otm <= 50:
                                ...
                                if data is not None and data["c"] < taking_price:
                                    self.entryOrder(data["c"], putSym, lotSize, "SELL", {"Expiry": expiryEpoch, "avg_premium": avg_premium, "avg_premium_1": avg_premium_1})
                    ...
                    try:
                        ...
                        if data_call_atm is not None and data_put_atm is not None:
                            ...
                            while otm <= 50:
                                ...
                                if data is not None and data["c"] < taking_price:
                                    self.entryOrder(data["c"], callSym, lotSize, "SELL", {"Expiry": expiryEpoch, "avg_premium": avg_premium, "avg_premium_1": avg_premium_1})
...
if __name__ == "__main__":
    ...
    devName = "AM"
    strategyName = "RMS_SS_doubleConfermation_Buy"
    version = "v1"

    startDate = datetime(2024, 1, 1, 9, 15)
    endDate = datetime(2025, 9, 25, 15, 30)
