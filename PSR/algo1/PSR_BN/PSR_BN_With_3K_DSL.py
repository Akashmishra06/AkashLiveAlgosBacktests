...
class algoLogic(optOverNightAlgoLogic):
    ...
    def run(self, startDate, endDate, baseSym, indexSym):
        ...
        straddleMultipleDrop = int(config.get('PSR_BN_With_3K_DSL', 'straddleMultipleDrop'))
        dailyStopLoss = int(config.get('PSR_BN_With_3K_DSL', 'dailyStopLoss'))
...
if __name__ == "__main__":
    ...
    devName = "AM"
    strategyName = config.get('PSR_BN_With_3K_DSL', 'algoName')
    version = "v1"

    startDate = eval(config.get('PSR_BN_With_3K_DSL', 'startDate'))
    endDate = eval(config.get('PSR_BN_With_3K_DSL', 'endDate'))
